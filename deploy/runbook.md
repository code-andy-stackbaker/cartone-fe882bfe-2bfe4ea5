...
|---|---|---|---|---|---|
| Backend API | `apps/backend` | Node 20+ | 8080 | `npm run build` (`tsc`) | `node dist/server.js` |
| Frontend SPA | `apps/frontend` | Node 20+ (static host) | 5173 | `npm run build` (`tsc --noEmit && vite build`) | `npx serve -s dist -l $PORT` |
...
|---|---|---|---|
| `PORT` | no | `8080` | Injected at deploy. |
| `ALLOWED_ORIGINS` | no | unset ⇒ CORS open for local dev | Set to the frontend origin, e.g. `http://localhost:5173`. Comma-separated. |
...
| Var | Required | Safe local default | Notes |
|---|---|---|---|
| `VITE_BACKEND_URL` | no | `http://localhost:8080` | Set from the deployed `BACKEND_URL`. No trailing slash. |
| `PORT` | no | `5173` | Port `serve` listens on. |
...
npm run build     # -> dist/
PORT=8080 ALLOWED_ORIGINS=http://localhost:5173 PAYMENTS_PROVIDER=mock node dist/server.js
```
...
curl -fsS http://localhost:8080/api/products    # expect the seeded catalogue JSON
```
...
npm install
npm test
VITE_BACKEND_URL=http://localhost:8080 npm run build    # -> dist/
PORT=5173 npx serve -s dist -l 5173
```

Verify: open <http://localhost:5173/> — the product catalogue renders (proving
the SPA reached the API), then add an item and complete the mock checkout; a
confirmation with an order id appears. `curl -fsS http://localhost:5173/`
...
2. Return the code to the last known-good commit and rebuild:
   ...
   cd apps/frontend && npm install && VITE_BACKEND_URL=http://localhost:8080 npm run build && npx serve -s dist -l 5173
   ```
...
docker run --rm -p 8080:8080 -e PORT=8080 -e PAYMENTS_PROVIDER=mock cartone-backend

docker build -f deploy/frontend.Dockerfile --build-arg VITE_BACKEND_URL=http://localhost:8080 -t cartone-frontend .
docker run --rm -p 5173:5173 -e PORT=5173 cartone-frontend
```
